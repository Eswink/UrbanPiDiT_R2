...
def main():
    ...
    for name,mod in [('coarse',m.coarse),('process_init',m.process_init),('reasoner',m.reasoner),('router',m.router),('urban',m.urban),('verifier',m.verifier)]:
        print(name, sum(p.numel() for p in mod.parameters())/1e6)

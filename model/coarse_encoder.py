...
class CoarseEncoder(nn.Module):
    def __init__(
        ...
        self.blocks=nn.ModuleList([
            WindowAttentionBlock(
                dim,heads,window_size,4.0,dropout,
                shift=bool(i%2),
    ...
    def forward(self,x:torch.Tensor):
        ...
        if T!=self.history_steps or C!=self.in_channels:
            raise ValueError(
                f'coarse 输入期望 T,C={self.history_steps},{self.in_channels}，'
                f'实际 {T},{C}'
            )
        z=self.patch(x.reshape(B,T*C,H,W))
        Ht,Wt=z.shape[-2:]
        tokens=z.flatten(2).transpose(1,2)
        for block in self.blocks:
            if self.use_checkpointing and self.training and tokens.requires_grad:
                tokens=checkpoint(
                    lambda q, b=block: b(q,(Ht,Wt)),
                    tokens,
                    use_reentrant=False,
                )
            else:

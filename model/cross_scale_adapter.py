...
class CrossScaleAdapter(nn.Module):
    def __init__(self,urban_dim:int,coarse_dim:int,process_dim:int,heads:int=8):
        super().__init__(); self.c=nn.Linear(coarse_dim,urban_dim); self.p=nn.Linear(process_dim,urban_dim); self.coarse_cross=CrossBlock(urban_dim,heads,2.0,0.0); self.process_cross=CrossBlock(urban_dim,heads,2.0,0.0)

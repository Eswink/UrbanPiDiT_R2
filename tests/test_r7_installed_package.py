...
def test_built_wheel_imports_from_clean_directory(tmp_path):
    ...
    modules=['train_r7_local','evaluate_r7_local','prepare_r7_local']
...
import train_r7_local, evaluate_r7_local, prepare_r7_local
import importlib.metadata as metadata
assert "installed" in str(Path(source.__file__).resolve())
entries=metadata.distribution("urbanpidit-r2-v6").entry_points
assert {e.name for e in entries}>={"urbanpidit-r7-train","urbanpidit-r7-evaluate","urbanpidit-r7-prepare"}
'''

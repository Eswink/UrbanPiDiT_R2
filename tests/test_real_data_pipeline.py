...
requires_real_fixture = pytest.mark.skipif(
    not (DYNAMIC.exists() and STATIC.exists()),
    reason=(
        "optional real-data fixture is not tracked in clean checkout; "
        "no synthetic fallback is permitted"
    ),
)
requires_dynamic_fixture = pytest.mark.skipif(
    not DYNAMIC.exists(),
    reason=(
        "optional UCI fixture is not tracked in clean checkout; "
        "no synthetic fallback is permitted"
    ),
)
...
def test_preprocess_uses_real_static_and_no_temporal_split_overlap(tmp_path: Path):
    ...
    for prev, nxt in [("train", "val"), ("val", "test")]:
        prev_end = max(
            datetime.fromisoformat(r["target_time"])
            for r in splits[prev]
        )
        next_start = min(
...
def test_worldcover_beijing_tile_and_url_are_deterministic():
    from data.download.worldcover_cog import (
        worldcover_cog_url,
        worldcover_tile_id,
    )

...
from .r7_process_forecast_lit_module import (
    R7ProcessForecastLightningModule,
)


def load_process_forecast_config(path:str|Path)->Dict[str,Any]:
    with open(path,"r",encoding="utf-8") as f:
        return yaml.safe_load(f)


def run_process_forecast(cfg:Dict[str,Any]):
    tc=cfg.get("train",{})
    ...
    ck=ModelCheckpoint(
        dirpath=tc.get(
            "ckpt_dir","outputs/checkpoints/r7_process_forecast"
        ),
        monitor="val/loss",
        mode="min",

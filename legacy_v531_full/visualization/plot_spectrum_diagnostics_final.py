...
def _apply_pub_style(fontsize: int = 11):
    plt.rcParams.update({
        "font.size": fontsize,
...
def _plot_radial_psd_grid(
    *,
    ...
    fig, axes = plt.subplots(
        nrows,
        ncols,
        figsize=(max(fig_w, 2.4 * ncols), max(2.6, row_h * nrows)),
        squeeze=False,
    )
    xs = np.arange(rmax + 1)
    ...
    for i, v in enumerate(vars_):
        for j, lead_lab in enumerate(lead_labels):
            ax = axes[i][j]
            ax.plot(xs, gt_psd[i, j], label="GT", linewidth=1.8)
            ax.plot(xs, a_psd[i, j], label=label_a, linewidth=1.8)
            if b_psd is not None:
                ax.plot(xs, b_psd[i, j], label=label_b, linewidth=1.8)
    ...
    if show_title:
        fig.suptitle(title, y=min(0.995, legend_y + 0.01))

    top_rect = 0.93 if show_title else 0.95
...
def _plot_band_error(
    *,
    ...
    ncols = len(lead_labels)
    fig, axes = plt.subplots(1, ncols, figsize=(max(fig_w, 3.0 * ncols), 4.0), squeeze=False)
    ...
    top_rect = 0.90 if show_title else 0.93
    plt.tight_layout(rect=(0, 0, 1, top_rect))
    Path(out_path).parent.mkdir(parents=True, exist_ok=True)
    plt.savefig(out_path, bbox_inches="tight")
    plt.close(fig)
...
def main():
    ...
    parser.add_argument("--label_a", type=str, default="w_fft")
    parser.add_argument("--label_b", type=str, default="wout_fft")
    ...
    parser.add_argument("--show_title", action=argparse.BooleanOptionalAction, default=False)
    parser.add_argument("--legend_y", type=float, default=0.985)
    parser.add_argument("--fig_w", type=float, default=10.2)
    parser.add_argument("--row_h", type=float, default=1.85)
    parser.add_argument("--fontsize", type=int, default=11)
    parser.add_argument("--out_path", type=str, required=True)
    args = parser.parse_args()

    _apply_pub_style(fontsize=int(args.fontsize))

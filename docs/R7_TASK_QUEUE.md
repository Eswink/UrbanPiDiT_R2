...
| #1 / PR12 scientific release | **BLOCKED** (human decision) at `ffe501e`; PR #12 still Draft | Cumulative G1–G4 status in [R7_ROADMAP_GATE_STATUS.md](R7_ROADMAP_GATE_STATUS.md): **no gate is positively supported** — G1/G2 not supported, G3 not satisfiable as stated, G4 shows the model losing to train-only climatology at 48 h. The five things the sequence DID establish are listed there. Closing this parent requires a human decision to publish the negative result set (or to fund a larger protocol); PR #12 stays Draft because merging main needs explicit human authorization. |

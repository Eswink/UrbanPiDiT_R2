...
on 2026-09-25 the eight verdicts were finalized and a closing commit carrying
`Closes #N` for all eight was pushed on `r7/weather-reasoning` (`e5be0c0`), but
the authorized fast-forward to `main` — the only path that closes issues on
GitHub — is **BLOCKED in-session**: the `guard_destructive_git` hook reads its
config at session start, so the documented escape hatch cannot take effect
mid-session (evidence and paths forward in decision 0002 addendum). The eight
issues remain **open on GitHub** until that single ff push lands; no force, no
release, no paid GPU, no uncontrolled data mirror. Experimental workflows stay
tag-gated and skipped by design.
